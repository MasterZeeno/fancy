...
termux_download() {
  ...
  echo
  if [[ $# != 2 ]] && [[ $# != 3 ]]; then
    FANCY_ARGS+=(+d --preset=failed)
    fancy_print --no-icon
    fancy_print -n "Invalid arguments for:"
    FANCY_ARGS+=(--no-icon)
    fancy_print +b -d "termux_download()"
    FANCY_ARGS+=(-n --no-padding)
    fancy_print "   expected:"
    for var in url destination checksum; do
      fancy_print ' '
      [[ $var =~ ^c ]] && fancy_print "["
      fancy_print "<"
      fancy_print -d "${var^^}"
    ...
    unset FANCY_ARGS; fancy_print 1>&2
    echo
    return 1
  fi
  local URL="$1"
  ...
  if [[ "$URL" =~ ^file://(/[^/]+)+$ ]]; then
    ...
    if [[ -d "$source" ]]; then
      ...
      FANCY_ARGS+=(--color=36)
      fancy_print -n +d "Downloading local source directory at:"
      fancy_print --print +b "'$source'"
      echo
      ...
      unset FANCY_ARGS
      fancy_print --preset=failed +d "No local source file found" 1>&2
      echo
      return 1
    fi
  fi
  ...
  local -a CURL_OPTIONS=(
    ...
    --no-progress-meter  # Don't print out transfer statistics
  )
  
  : "${TMPDIR:=/tmp}"
  : "${TERMUX_PKG_TMPDIR:=$TMPDIR}"
  
  TMPFILE=$(mktemp "$TERMUX_PKG_TMPDIR/download.${TERMUX_PKG_NAME-unnamed}.XXXXXXXXX")
  trap 'rm -f "$TMPFILE"' EXIT
  
  FANCY_ARGS=(--no-print --color=36)
  fancy_print -n +d "Downloading:"
  fancy_print --print +b "'$(basename "$DESTINATION")'"
  echo
  ...
  if ! curl "${CURL_OPTIONS[@]}" --output "$TMPFILE" "$URL"; then
    ...
    for (( try=1; try <= retry; try++ )); do
      FANCY_ARGS=(-n +d --no-print --preset=warn)
      fancy_print "Retrying"
      FANCY_ARGS+=(--no-icon)
      fancy_print -d +b "($try/$retry)"
      fancy_print "in"
      fancy_print -d +b "$delay"
      fancy_print "seconds"
      unset FANCY_ARGS; fancy_print
      echo
      sleep "${delay}"
      if curl "${CURL_OPTIONS[@]}" --output "$TMPFILE" "$URL"; then
        error=0
    ...
    if [[ "${error}" != 0 ]]; then
      FANCY_ARGS=(--no-print --preset=failed)
      fancy_print -n +d "Failed to download:"
      fancy_print --no-icon --print +b "'$(basename "$DESTINATION")'" 1>&2
      echo
      return 1
    fi
  fi
  ...
  if [[ -z "$CHECKSUM" ]]; then
    FANCY_ARGS=(+d --no-print --preset=warn)
    fancy_print -n "No checksum check for:"
    FANCY_ARGS+=(--no-icon)
    fancy_print -d +b "'$(basename "$DESTINATION")'"
    fancy_print -n "   Actual:"
    fancy_print --print -d +b "'$ACTUAL_CHECKSUM'"
    echo
  elif [[ "$CHECKSUM" == "SKIP_CHECKSUM" ]]; then
    :
  elif [[ "$CHECKSUM" != "$ACTUAL_CHECKSUM" ]]; then
    FANCY_ARGS=(--no-print --preset=failed)
    fancy_print -n +d "Wrong checksum for:"
    FANCY_ARGS+=(--no-icon)
    fancy_print +b "'$(basename "$DESTINATION")'"
    fancy_print -n +d "   Expected:"
    fancy_print +b "'$CHECKSUM'"
    fancy_print -n +d "   Actual:"
    fancy_print --print +b "'$ACTUAL_CHECKSUM'" 1>&2
    echo
    return 1
  fi
